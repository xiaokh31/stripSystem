...
## WAGE-HOURS-07 Closure Evidence (2026-07-23)

- Migration `20260723193000_attendance_import_audited_deletion` passed on the
  existing database and a fresh 36-migration database. PostgreSQL rejected a
  second active SHA, accepted a new id after tombstoning the first, and the
  browser concurrency gate produced exactly one `201` and one `409`.
- Whole-import deletion is a single locked transaction across immutable event,
  revision and current generated-file supersede. Unit races cover queued/running
  jobs plus parse/generate late persistence; deleted routes consistently return
  `ATTENDANCE_IMPORT_DELETED`.
- Docker gates passed: API lint/typecheck/build, 46 suites / 359 unit and
  21 suites / 126 E2E; Web lint/typecheck/build and 271 unit; Worker 183 pytest.
  The full real nginx/PostgreSQL/Worker Chromium work-hours spec passed 6/6.
- Browser evidence covers HR success, ADMIN success, read-only and warehouse
  manager `403`, required reason, Cancel, double-submit, fallback, stale URL,
  refresh, immutable actor snapshot after rename/disable, same-SHA reupload,
  original/generated byte preservation and no history download actions.
- Four high-signal full-resolution screenshots under
  `test-results/wage-hours-07/browser/` cover English light desktop dialog,
