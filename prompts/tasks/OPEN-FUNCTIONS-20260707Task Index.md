...
- P0 现场回归 `UNLOAD-REPORT-01` 已按业务决定 A 完成当时的仓库实现和自动化；但 2026-07-25
  现场再次确认长单元格没有可靠自适应行高，底部备注在工作表可见但打印仍裁切。新增
  `UNLOAD-REPORT-02` 作为权威修复任务，01 的“无已知裁切问题”不再代表当前现场结论。
...
当前执行队列（2026-07-26 MDT 复核）：

### A0. 2026-07-25 新增开发线路
...
4. `POD-01Versioned Template Registry API RBAC and Audit.md` — `BLOCKED BY POD-00`
5. `POD-02Template Management Web Workspace.md` — `BLOCKED BY POD-01`
6. `POD-03Document Revision Generation Archive API.md` — `BLOCKED BY POD-02`
7. `POD-04Composer Print and Archive Web Workflow.md` — `BLOCKED BY POD-03`
8. `POD-05Full Stack I18n Print Archive Exit Gate.md` — `BLOCKED BY POD-04`
...
3. `UNLOAD-REPORT-02Adaptive Cell Height and Printed Notes Regression.md` 外部关闭项
   - 先在 A0 完成新回归的仓库实现和 Docker visual。随后在办公室
     Windows/Microsoft Excel 使用真实 API 下载文件完成 Print Preview 与 Print to PDF，
     确认长单元格和底部备注逐页完整后才能 Done。
...
Pilot 前必须验收：
1. `UNLOAD-REPORT-02`：长单元格自适应、底部备注、Windows/Microsoft Excel
   Print Preview 与 Print to PDF 无裁切签字；02 supersede 01 的旧外部门禁。
