...
- PARSER-PROFILE-01/02/03/04/05/06/07 已完成；PARSER-PROFILE-08 仓库实现和当前环境自动化已完成，状态为
  `CODE_COMPLETE_EXTERNAL_VERIFICATION_PENDING`：领域 schema、learning-case 关系、Worker bounded inspection、可解释结构指纹、
  ...
  replay diff、inspection/accessibility/stable-code 与 runtime artifact 缺口已完成首轮 remediation；随后 targeted review
  发现的 empty-sheet 检测、lookup error focus/ARIA 与 replay raw enum 三项残余也已通过 fresh supervised remediation、
  235 项 Web unit 和完整 Docker Chromium/视觉矩阵关闭；终态双轴审查补充发现的 preview package raw enum 与
  stale preview failure action-message race 也已修复并复跑相同门禁；随后 PARSER-PROFILE-05 已交付首次拆柜完成快照、
  durable completion replay outbox、批准资格、ACTIVE + REVIEW_REQUIRED + 0/3、pause/resume/retire/fork、精确 RBAC 及
  双语治理工作区，并通过 29 个空库迁移、API/Web/Worker 全量门禁和真实工作簿 Chromium 视觉验收；随后
  ...
  该 parser Task 不再重复启动；其外部数据验收独立保留，不阻塞当前公网开发线路。
- P0 现场回归 `UNLOAD-REPORT-01` 已按业务决定 A 完成当时的仓库实现和自动化；2026-07-25
  的长单元格和底部备注回归由 `UNLOAD-REPORT-02` 完成了首轮仓库修复。但 2026-07-28
  办公室实际打印再次确认左侧白色留边消失，并发现生成报告存在目的仓缺失。02 的
  “只剩外部签字”结论已失效；产品同时明确 `0–16` 条正常报告必须保留全部合法
  目的仓槽位的可用容量并保持一 sheet/一页，不得按估算高度提前分页。
  `UNLOAD-REPORT-03` 已完成仓库实现、Docker full-stack/package/PDF/PNG 和当前
  环境自动化，现只等待 Windows/Microsoft Excel 与实际打印外部门禁。
- Wage / Unloading Wage 既有线路已完成；2026-07-25 新增 `UNLOAD-WAGE-14`，把美转加
  ...
  2026-07-21 新增的 `WAGE-HOURS-01` 至 `05` 已于 2026-07-22 全部
...
- 2026-07-25 新增 POD 大功能及 `POD-00` 至 `05`。菜单固定为 `POD`；办公室人员可维护
  ...
  format/field/print profile；没有样本不得用自造模板开始 POD-01。全线路 strict en/zh-CN、RBAC、
  original bytes、generated file、audit、storage containment 和 Docker print visual 为硬门禁。
- 2026-07-22 新增公网访问线路 `PUBLIC-DEPLOY-01` 至 `03`；`PUBLIC-DEPLOY-01` 公网安全基线已于
  2026-07-22 MDT 完成。`PUBLIC-DEPLOY-02` 的 Cloudflare named-tunnel local-canonical 仓库实现和当前环境自动化
  也已完成，仅等待真实 domain/account、Access MFA、非公司网络和 token rotate/revoke 外部激活证据；本地 Docker
  继续是唯一 writer。产品短暂选择后又撤回 OCI 路线，`PUBLIC-DEPLOY-03` 已归档且其未完成修改已全部还原。
  任何路线都不得公开 PostgreSQL/Redis，不得 local/cloud active-active，并须保持 strict en/zh-CN。
...
- 2026-07-21 新增 Work Hours 修正线路 `WAGE-HOURS-01` 至 `05`：01/02/03/04 已完成 odd first/last、even paired intervals、
  fixed lunch only once、`wage-attendance-v2`、auditable method/interval persistence，以及 `.xls` 逐格 XF/formula
  保真、特殊 Sheet/adjustment row 保护和 bounded adaptive dimensions；02 的复审门禁已补齐可靠 employee id/name
  独立匹配、name token 最短长度、BIFF date/full-date slot 验证、正数备注保护及逐 Sheet headers/target/key-style inventory，
  Docker 定向 17、Worker 181 tests 通过；03 已按稳定员工 identity 显示完整 30/31 天、API summary、localized
  method/issues 并移除全局 first-100 truncation，Web 262 tests、focused Chromium 4/4 和 6 张逐图检查通过；04 已交付
  ...
  API/数据库/storage 留存。
- 2026-07-22 新增 `WAGE-HOURS-07Attendance Import Audited Deletion.md`：整次 attendance import 采用
  import-level tombstone + immutable deletion event；`attendance.imports.delete` 仅默认授予 ADMIN/HR_MANAGER；
  active-only SHA 唯一约束、活动 job/sync worker race、防旧 URL 继续操作、删除历史和 strict en/zh-CN 均为
  Definition of Done。该 Task 已于 2026-07-23 完成全部当前环境实现、36 migration、API/Web/Worker 与真实
  Chromium 6/6 门禁，状态 `DONE`。
...
- `NATIVE-AUTH-01` repository、Docker 自动化、真实数据库/HTTP 和 iOS 非破坏性已认证矩阵已完成；三次 supervisor
  ...
  打印/Excel、Android/iOS 人工设备矩阵或目标部署主机；`WAGE-HOURS-07` 已于 2026-07-23 完成，并有
  5 个 Windows 原生/MSIX P6 Task 已归档。
...
当前执行队列（2026-07-28 MDT 复核）：

### A0. 2026-07-25 至 2026-07-28 新增开发线路

严格按以下顺序，每个 Task 使用一个 fresh supervisor Session：

1. `UNLOAD-WAGE-14Optional Trailer Number for US-to-Canada Transfer.md` — `DONE`
...
2. `UNLOAD-REPORT-02Adaptive Cell Height and Printed Notes Regression.md` —
   `SUPERSEDED_BY_UNLOAD_REPORT_03`
   - 02 的自适应行高和 Standards 历史证据继续保留，但 2026-07-28 外部验收发现
     左侧打印留边、目的仓完整性和 height-based early pagination 回归，原终态已
     失效。不得重跑 02 或直接补签字。
3. `UNLOAD-REPORT-03Print Margin and Destination Preservation Regression.md` —
   `CODE_COMPLETE_EXTERNAL_VERIFICATION_PENDING`
   - 保存后 `N/O/P` 逐条守恒、16 槽纸面顺序、0–16 单页、17+ 容量分页、
     fail-closed、旧成功文件保护、strict i18n 及 Docker
     full-stack/package/PDF/PNG 已完成。20 张 generated pages 左侧 whitespace
     均为 22.225mm，与模板 delta 0mm；只剩 Windows/Microsoft Excel Print
     Preview、Print to PDF 和办公室实际打印签字。
4. `POD-00Real Template Contract and Worker Proof.md` — `INPUT REQUIRED`
   - 只有业务提供真实/明确脱敏 blank template、matching approved completed/printed
     example、字段和打印规则后才执行。不得用 synthetic template 冒充真实 Phase 0。
5. `POD-01Versioned Template Registry API RBAC and Audit.md` — `BLOCKED BY POD-00`
6. `POD-02Template Management Web Workspace.md` — `BLOCKED BY POD-01`
7. `POD-03Document Revision Generation Archive API.md` — `BLOCKED BY POD-02`
8. `POD-04Composer Print and Archive Web Workflow.md` — `BLOCKED BY POD-03`
9. `POD-05Full Stack I18n Print Archive Exit Gate.md` — `BLOCKED BY POD-04`

POD 线路不得跳过 00，也不得把 01-05 合并进同一 Session。所有新增 UI/API 均以
strict `en` / `zh-CN`、stable code/labelKey、权限、审计、original/generated bytes
留存和失败安全 cleanup 为 release gate。

### A. 既有开发机任务终态（不得作为下一任务）

1. `WEB-DASHBOARD-09Future Month Contamination and Clock Integrity Regression.md` — `DONE`
   - 已确认 host/API/Web container 当前时间均为 2026-07-24；2099 直接来自
     `apps/web/e2e/unloading-wage.spec.ts` 的硬编码月份/完成时间及缺失 teardown，遗留
     `PC-TRAILER-TR-E2E-*` pay-container 后又被 Dashboard `defaultMonth()` 作为全库最大
     `completedAt` 采用。
   - 已交付共用 server clock/future-date policy、Edmonton 月份边界、两个完成入口五分钟容差与
     无副作用拒绝、future-date operations review、Dashboard/summary/wage 数据库上限谓词及
     server baseline + monotonic 页眉时钟。
   - 唯一 2099 fixture 经多重 provenance 证明后精确清理，非 fixture 指纹不变；工资 E2E
     改用专用历史月、唯一前缀、Playwright finally + shell trap、故意失败探针和残留审计。
     真实 Chromium 成功组 2/2、六张原分辨率逐图检查及 fixture/账号/storage 零残留均通过。
2. `WEB-DASHBOARD-07Dashboard Drilldown Navigation and Target Filters.md` — `DONE`
   - 已统一 Dashboard aggregate count 与目标列表 predicate，为 imports、containers、inventory、load jobs、
     ...
     清理均通过。
3. `WEB-DASHBOARD-08Dashboard Drilldown Full Stack I18n Visual Exit Gate.md` — `DONE`
...
1. `PUBLIC-DEPLOY-01Public Internet Security Baseline.md` — `DONE`
   - 已交付 public mode fail-closed、可撤销 secure HttpOnly browser session、CSRF/Origin、Redis rate limit、trusted
     proxy、安全 headers/health/error 脱敏、审计、严格 en/zh-CN、provider-neutral Compose/network contract 及迁移。
     Docker API 350 unit/125 E2E、Web 266 unit/build、Worker 183 tests、Chromium 业务/角色/尺寸矩阵、35 个迁移和
     公网运行时/危险配置拒绝门禁均通过；未创建任何云资源。
2. `PUBLIC-DEPLOY-02Cloudflare Tunnel Local Canonical Pilot.md` — `CODE COMPLETE / EXTERNAL ACTIVATION PENDING`
   - 已交付 pinned named-tunnel profile、secret-file/fail-closed/network contract、nginx no-store 与稳定 413、50 MB
...
import deletion blocker；`PARSER-PROFILE-02` 已完成 bounded inspection、结构指纹、strict mapping/provenance、
真实 fixture equivalence、性能与安全门禁；`PARSER-PROFILE-03` 已完成 inspect/draft/preview/replay/diff/artifact/
DRAFT candidate API、revision/idempotency/storage/RBAC/i18n 门禁；`PARSER-PROFILE-04` 已完成 Office wizard、
failed-import/manual linkage、revision/latest-write-wins、业务 replay diff、inspection/accessibility、stable-code/i18n 与
runtime artifact 收口；targeted review 的 empty-sheet、lookup error focus/ARIA 与 replay raw enum 残余已完成 focused
remediation 和 Docker Web/Chromium 门禁；`PARSER-PROFILE-05` 已完成统一 completion snapshot/outbox、固定 completion replay、approval eligibility、
...
3. `UNLOAD-REPORT-03Print Margin and Destination Preservation Regression.md` 外部关闭项
   - 仓库实现和当前环境自动化已完成，不得重启开发或重跑 02。在办公室
     Windows/Microsoft Excel 使用 03 最新 API 下载文件完成逐目的仓、单页、
     Print Preview、Print to PDF 和实际纸张打印；确认左侧白边、目的仓、
     托数/箱数和底部备注后才能 Done。
...
Pilot 前必须验收：
1. `UNLOAD-REPORT-03`：目的仓从 expected plans 到最终 API 下载文件逐条守恒，
   `0–16` 条正常数据保留完整 16 槽可用容量、未用槽位为空，并保持一 sheet/一张
   A4；左侧打印白边不低于模板基线容差，并通过 Windows/Microsoft Excel Print
   Preview、Print to PDF 和实际纸张打印；03 supersede 02 的失败外部门禁。
...
不需要新开任务的项目：
- Account/RBAC：完成。
- Unloading wage 既有线路：完成；2026-07-25 的托车号可选变更只执行新
  `UNLOAD-WAGE-14`，不得重跑旧 01-13。
...
- WEB-DASHBOARD-00 至 09 的设计、API、Shell、首页、i18n/视觉、精确 drilldown、全点击退出矩阵及日期/时钟完整性回归均已完成；这些 Task 不得重复启动。
...
3. `UNLOAD-REPORT-03` 的仓库实现和 Docker 门禁已完成，不得重跑 02 或再次启动
   03 开发。下一动作只是在办公室 Windows/Microsoft Excel 使用 03 最新 API
   下载文件完成逐目的仓、单页、Print Preview、Print to PDF 和实际纸张打印签字；
   外部门禁通过后把 03 更新为 Done。
...
5. `WEB-DASHBOARD-09` 已达到 `DONE`，不得重跑。后续工资 E2E 必须使用
   `scripts/run-web-dashboard-09-e2e.sh`，不得直接运行旧 spec 绕过 cleanup supervisor；
   精确清理工具不得扩展为按未来年份广泛删除业务记录。
6. `PUBLIC-DEPLOY-01` 已达到 `DONE`；`PUBLIC-DEPLOY-02` 已完成仓库实现和当前环境自动化，不得重启开发 Task。
...
7. `PUBLIC-DEPLOY-03` 已归档，不得执行或恢复其 OCI/ARM64 修改；恢复需要产品批准并同步任务状态。
8. `WAGE-HOURS-01/02/03/04/05/06/07` 已关闭，不得重跑；最终 workbook visual gate 的 BIFF/LibreOffice/Chromium
...
9. WEB-BRAND-01/02/03/04 已达到 DONE，不得重跑；后续品牌需求必须另立 Task，不得在本线路继续扩大到
   PWA 192/512 icon、Native 或 Excel/PDF/label branding。
10. `WEB-DASHBOARD-00` 至 `09` 与 `WEB-OPS-01/02/03/04/05/06/07/08/09` 均保持关闭，
   不重复启动；后续日期完整性需求必须另立 Task。
11. PARSER-PROFILE-01 至 07 已完成，08 已完成仓库实现和当前环境自动化，只等 4 组真实/明确脱敏 golden pair 与业务签字；
...
12. PUBLIC-DEPLOY 之外的既有 Task 仍由外部条件决定：有真实/脱敏包装 workbook 时执行 `UNLOAD-PALLET-04`；只有
   iOS/Android 设备时完成 NATIVE-AUTH/UX 外部证据；有目标部署主机且其他活动 gate 已关闭时执行 `P5-PILOT-01`。
13. Android/iOS release 实机可采集主题、标题、冷启动和双语扫码证据，并按 B-5 至 B-8 关闭活动 Native gate；
   不等待或启动 Windows App。
14. 真实/脱敏业务 workbook 到位后执行 `UNLOAD-PALLET-04`；复用同一数据和目标打印机/PDA关闭
   `UNLOAD-PALLET-10` 的外部签字。08/09/10 代码任务均已完成，不要重复建立规则或重新跑开发任务。
15. P6-MOBILE-09 至 13 已归档，任何主机都不得执行。恢复需要产品批准、移除归档标记、恢复关联验收范围并同步索引/报告。
16. 所有上述外部打印、真实样本、POD exit gate 和 Android/iOS 设备 gate 结束后，最后执行 `P5-PILOT-01`。PUBLIC-DEPLOY-03 OCI
    canonical host 已归档，不属于当前 pilot gate；恢复须先取得产品批准并同步任务状态。
17. `UNLOAD-INVENTORY-02`、`UNLOAD-WAGE-13`、`UNLOAD-PALLET-09`、`DOCKER-CACHE-01` 已完成，不得重复执行。
