...
2. `WEB-DASHBOARD-09` 已达到 `DONE`，不得重跑。后续工资 E2E 必须使用
...
4. `PUBLIC-DEPLOY-03` 已归档，不得执行或恢复其 OCI/ARM64 修改；恢复需要产品批准并同步任务状态。
5. `WAGE-HOURS-01/02/03/04/05/06/07` 已关闭，不得重跑；最终 workbook visual gate 的 BIFF/LibreOffice/Chromium
...
7. `WEB-DASHBOARD-00` 至 `09` 与 `WEB-OPS-01/02/03/04/05/06/07/08/09` 均保持关闭，
...
10. PUBLIC-DEPLOY 之外的既有 Task 仍由外部条件决定：有真实/脱敏包装 workbook 时执行 `UNLOAD-PALLET-04`；只有
   iOS/Android 设备时完成 NATIVE-AUTH/UX 外部证据；有目标部署主机且其他活动 gate 已关闭时执行 `P5-PILOT-01`。
11. Android/iOS release 实机可采集主题、标题、冷启动和双语扫码证据，并按 B-5 至 B-8 关闭活动 Native gate；
   不等待或启动 Windows App。
12. 真实/脱敏业务 workbook 到位后执行 `UNLOAD-PALLET-04`；复用同一数据和目标打印机/PDA关闭
   `UNLOAD-PALLET-10` 的外部签字。08/09/10 代码任务均已完成，不要重复建立规则或重新跑开发任务。
13. `UNLOAD-REPORT-01` 只剩 Microsoft Excel Print Preview / Print to PDF，完成外部检查后记录结果即可，
   不要重复开发。
14. P6-MOBILE-09 至 13 已归档，任何主机都不得执行。恢复需要产品批准、移除归档标记、恢复关联验收范围并同步索引/报告。
15. 所有上述外部打印、真实样本和 Android/iOS 设备 gate 结束后，最后执行 `P5-PILOT-01`。PUBLIC-DEPLOY-03 OCI
    canonical host 已归档，不属于当前 pilot gate；恢复须先取得产品批准并同步任务状态。
16. `UNLOAD-INVENTORY-02`、`UNLOAD-WAGE-13`、`UNLOAD-PALLET-09`、`DOCKER-CACHE-01` 已完成，不得重复执行。
