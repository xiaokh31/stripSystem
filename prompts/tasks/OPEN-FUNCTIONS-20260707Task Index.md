...
- P0 现场回归 `UNLOAD-REPORT-01` 已按业务决定 A 完成当时的仓库实现和自动化；2026-07-25
  ...
  `UNLOAD-REPORT-03` 已完成报告内容、目的仓守恒、打印几何和当前环境自动化；
  但 2026-07-29 又确认同一柜号重新生成报告会在普通文件区域追加历史文件，且报告
  与面单缺少统一的“每柜每类型唯一当前文件、失败不覆盖”约束。03 原“只等待外部
  打印”终态失效。P0 `UNLOAD-REPORT-04` 已完成当前文件替换：成功重新生成替换
  对应 current slot，文件区域最多一份当前报告和一份当前面单，历史仅保留审计；
  数据库唯一约束、repair、失败/并发守恒和 full-stack/视觉门禁均已通过。
...
- 2026-08-01 新增三个 P0 现场回归 Task。`FILE-UPLOAD-01` 已完成卸柜/考勤 multipart
  ...
  Generate 失败”的异步工资表闭环，用户已于 2026-08-02 明确验收通过并标记 `DONE`。
  随后单独发现的 A 列周末底色问题由新增 `WAGE-HOURS-09` 修复，当前环境门禁与用户
  专项验收也已通过，状态同为 `DONE`；
...
2. `WAGE-HOURS-08Parsed Attendance Wage Workbook Generation Regression.md` —
   ...
   - 已从结构合同创建独立、确定性、无历史业务值的 tracked legacy BIFF 模板；API/Worker
     ...
     BIFF/LibreOffice、隐私、cleanup 和全量 Docker 门禁均通过；用户于 2026-08-02
     明确确认本 Task 验收通过。后续周末底色细节已拆为 09，不重开或重跑 08。
...
3. `UNLOAD-REPORT-03Print Margin and Destination Preservation Regression.md` —
   ...
   - 保存后 `N/O/P` 逐条守恒、16 槽纸面顺序、0–16 单页、17+ 容量分页、
     ...
     均为 22.225mm，与模板 delta 0mm；但 2026-07-29 发现成功重生成会追加普通
     文件记录；随后又确认 8 条工件错误占用了白色追加行，不能先补打印签字或
     标记 Done。
4. `UNLOAD-REPORT-04Current Report and Label Replacement Regression.md` — `DONE`
   - 同一柜号每个业务文件类型最多一个 current：成功重生成替换当前拆柜报告或
     当前托盘面单，失败/并发/重试不覆盖旧成功文件；普通文件区域固定两个槽位，
     历史/失败/技术工件不显示但继续审计。必须包含数据库唯一约束、既有重复数据
     repair、storage staging、BullMQ/full-stack、strict i18n/no-flash 已通过。
     04 未修改 Excel row layout；验证报告见
     `docs/reports/unload-report-04-current-artifact-replacement-verification.md`。
     既有生产重复 current 的清理固定使用默认 dry-run 的
     `repair:current-generated-files` 和
...
7. `POD-01Versioned Template Registry API RBAC and Audit.md` — `BLOCKED BY POD-00`
8. `POD-02Template Management Web Workspace.md` — `BLOCKED BY POD-01`
9. `POD-03Document Revision Generation Archive API.md` — `BLOCKED BY POD-02`
10. `POD-04Composer Print and Archive Web Workflow.md` — `BLOCKED BY POD-03`
11. `POD-05Full Stack I18n Print Archive Exit Gate.md` — `BLOCKED BY POD-04`
...
的工资表生成闭环和 WAGE-HOURS-09 的 A 列周末底色修复均已由用户验收并达到 `DONE`。
外部样本、设备、打印/Excel、目标主机和
...
九项均已分别完成并不得重跑。08 已交付确定性脱敏模板、
只读镜像供应、clean tracked checkout preflight、真实 7 月 full-stack/BIFF/LibreOffice
门禁并通过用户验收；09 已把周末样式改为按实际日期判定，补齐语义 BIFF/视觉门禁，
并于 2026-08-02 通过用户专项验收。
...
Pilot 前必须验收：
1. `UNLOAD-REPORT-04` + `UNLOAD-REPORT-05`：04 保证普通文件区域每柜最多一份
   当前拆柜报告和一份当前托盘面单，成功重生成替换对应 slot，失败/并发/重试保留
   旧 current；05 保证每页 `1–8` 条只使用深色主行、`9–16` 条才按纸面顺序启用
   白色追加行，17+ 按 16 分页。两者共同继承 03 的逐项守恒、一 sheet/一张 A4、
   左侧打印白边和 Standards，并通过 Windows/Microsoft Excel Print Preview、
   Print to PDF 和实际纸张打印。
...
10. `WAGE-HOURS-01/02/03/04/05/06/07/08/09` 已关闭，不得重跑。最终 workbook visual gate 的 BIFF/LibreOffice/Chromium
