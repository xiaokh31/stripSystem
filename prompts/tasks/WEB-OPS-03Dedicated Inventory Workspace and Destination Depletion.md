...
- 已新增 canonical `/inventory` 顶层工作区，Inventory 菜单位于 Containers 与 Load Jobs 之间并受
  `inventory.read` 控制；Dashboard、Reports 和快捷入口统一指向新路由。旧 `/reports/inventory` 使用永久重定向，
  无损保留 containerNo、destinationCode、status 和 containerId。
- 页面使用稳定 containerId 选择柜子，读取现有 container summary、destination aggregate、container detail summary
  和 adjustment history。人工消库存继续复用现有 destination-scoped API、reason/error mapping 和共享组件；按钮受
  `inventory.adjust` 控制，对话框显示柜号、目的仓、当前值、仅作预览的预计剩余、理由和备注，并实现 Escape、焦点圈和
  关闭后焦点恢复。
- 未新增 endpoint、Prisma schema 或 migration。API 仅扩展现有 container detail summary 的历史可见性：完全
  `ADJUSTED_OUT` 的目的仓仍保留在指定柜子的审计明细中；全局 active aggregate 口径不变。
- 成功 mutation 后由 API 重读当前工作区，并通过现有 inventory sync 事件、localStorage/BroadcastChannel 和全局 listener
  刷新已打开的 Dashboard、柜子详情与库存页；前端预计值不作为库存事实来源。
- Docker Chromium 唯一前缀夹具证据：指定柜子/目的仓从 active/remaining `3/3` 变为 `2/2`，adjusted out
  `0 -> 1`，loaded 保持 `0`；只提交 1 次 adjustment，历史含真实 actor、`SCAN_MISSED`、note、pallet eventId 和
  `LABEL_PRINTED -> ADJUSTED_OUT`。只读用户无按钮，无 inventory.read 用户菜单隐藏、页面拒绝且 mutation API 返回 403。
- E2E finally 已精确清理 container、destination、pallet、adjustment/event、测试用户/角色与 storage path；复核
  `WEBOPS03-` container/user/role/storage 残留均为 0。mutation 证据位于
  `test-results/web-ops-03/inventory-mutation-evidence.json`。
- en/zh-CN、light/dark、390/768/1366/1920/2560 与真实 200% browser zoom 门禁通过；24 张 PNG 位于
  `test-results/web-ops-03/`，代表性六张已按原始分辨率人工检查，无混语、裁剪、重叠或页面级横向溢出。
- 验证通过：API lint、typecheck、26 suites / 221 unit、15 suites / 92 E2E；Web lint、typecheck、197 unit、
  production build；WEB-OPS-03 Playwright Chromium 1/1；full-stack healthcheck 与 `git diff --check`。
