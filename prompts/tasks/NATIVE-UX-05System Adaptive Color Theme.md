...
- 活动验收平台为 Android 和 iOS。
- Windows app theme、title bar、高对比、scanner chrome 和 MSIX 实机矩阵已随 Windows 原生安装包路线归档，
  不再是本 Task 的 release blocker。
- 既有跨平台主题代码保持不变；恢复 Windows 验收必须先解除 P6-MOBILE-09 至 13 及索引/报告中的归档状态。
...
- Android `styles.xml` / manifest、iOS `Info.plist` / launch screen
...
3. 本任务不要求跟随 Android Material You 或任意系统壁纸提取色。
...
- 已归档：Windows 应用/系统 theme、title bar、窗口背景、高对比模式和焦点可见性验收。
...
- 200% 字体缩放、320px 手机宽度和平板横屏无重叠或不可见操作。
...
1. Android、iOS 分别以 release 构建启动 light 和 dark 模式。
...
6. Android 额外验证 force dark；iOS 额外验证系统自动日落切换。Windows 高对比矩阵已归档。
...
6. Android/iOS 无法完成的实机项必须保留为 release blocker；已归档 Windows 项不得阻塞当前完成状态。
